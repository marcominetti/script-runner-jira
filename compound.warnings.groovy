...
def createAnomaly(List<Map<String,String>> anomalies, Issue issue, String level, String description) {
    ...
    anomaly.put("resolution", issue.getResolution() != null)
...
def calculateAnomalies(Issue issue, List circularityCache, IssueLinkManager issueLinkManager, Logger log, List<Map<String,String>> anomalies) {
    ...
    if (childCount == 0) {
        ...
        if (thisRemainingEstimate != null && thisRemainingEstimate > 0 && resolution != null) {
            createAnomaly(anomalies, issue, "WARN", "issue risolta ma con remaining estimate: il remaining estimate non viene considerato")
        }
    } else {
        if (thisOriginalEstimate != null && thisOriginalEstimate > 0) {
            createAnomaly(anomalies, issue, "WARN", "issue padre ma con original estimate: l'original estimate non viene considerato, rimuoverlo ed assicurare di averlo distribuito correttamente tra i figli")
        }
        if (thisTimeSpent != null && thisTimeSpent > 0) {
            createAnomaly(anomalies, issue, "WARN", "issue padre ma con worklog: il time spent non viene considerato")
        }
        if (thisRemainingEstimate != null && thisRemainingEstimate > 0) {
            createAnomaly(anomalies, issue, "WARN", "issue padre ma con remaining estimate: il remaining estimate non viene considerato")
        }
    }
}
